...
public class PlayerController : MonoBehaviour
{
    ...
    void Start()
    {
        ToggleCursor(false); // 게임 시작 시 커서 잠금
    }
    ...
    {
        Move();
        Debug.DrawRay(transform.position + (transform.forward * 0.2f) + (transform.up * 0.11f), Vector3.down);
        Debug.DrawRay(transform.position + (-transform.forward * 0.2f) + (transform.up * 0.11f), Vector3.down);
        Debug.DrawRay(transform.position + (transform.right * 0.2f) + (transform.up * 0.11f), Vector3.down);
        Debug.DrawRay(transform.position + (-transform.right * 0.2f) + (transform.up * 0.11f), Vector3.down);
    }
    ...
    public void OnInventoryInput(InputAction.CallbackContext context)
    {
        ...
        {
            if (UIManager.Instance.UIStack.Count !=0&&UIManager.Instance.UIStack.Peek() == UIKey.InventoryUI)
            {
            ...
            // 인벤토리 창이 열렸는지 확인
            bool isOpen = InventoryUI.Instance != null && InventoryUI.Instance.inventoryWindow.activeSelf;
    ...
    bool IsGrounded()
    {
        ...
        Ray[] rays = new Ray[4]
        {
            new Ray(transform.position + (transform.forward * 0.2f) + (transform.up * 0.11f), Vector3.down),
            new Ray(transform.position + (-transform.forward * 0.2f) + (transform.up * 0.11f), Vector3.down),
            new Ray(transform.position + (transform.right * 0.2f) + (transform.up * 0.11f), Vector3.down),
            new Ray(transform.position + (-transform.right * 0.2f) +(transform.up * 0.11f), Vector3.down)
